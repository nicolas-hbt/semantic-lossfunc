...
class Tester:
    ...
    def test(self):
        ...
        sem_t_triples_ext = {}
        for b in range(1, self.args.buckets + 1):
            ext_CWA_h[b] = {'sem1_h': 0.0, 'sem3_h': 0.0, 'sem10_h': 0.0}
            ext_CWA_t[b] = {'sem1_t': 0.0, 'sem3_t': 0.0, 'sem10_t': 0.0}
            sem_h_triples_ext[b] = 0
            sem_t_triples_ext[b] = 0
            ...
            for triple in tqdm(X_tails):
                ...
                if self.metric == 'sem' or self.metric == 'all':
                    ...
                    if self.sem == 'extensional' or self.sem == 'both':
                        if r.item() in self.dataset.r2ts.keys():  # s'assurer d'ajouter triplets inverses
                            if len(self.dataset.r2ts[r.item()]) >= 10:
                                s1, s3, s5, s10 = self.sem_at_k_ext(
                                    indices_tail[:10], r.item(), side='tail', k=10)
                                ext_CWA_t[bucket]['sem1_t'], ext_CWA_t[bucket]['sem3_t'], ext_CWA_t[bucket]['sem10_t'] = ext_CWA_t[bucket]['sem1_t'] + \
                                    s1, ext_CWA_t[bucket]['sem3_t'] + s3, ext_CWA_t[bucket]['sem10_t'] + s10
                                sem_t_triples_ext[bucket] += 1
                    ...
                    if self.sem == 'extensional' or self.sem == 'both':
                        if r.item() in self.dataset.r2ts.keys():
                            if len(self.dataset.r2ts[r.item()]) >= 10:
                                s1, s3, s5, s10 = self.sem_at_k_ext(
                                    indices_tail[:10], r.item(), side='tail', k=10)
                                ext_CWA_h[bucket]['sem1_h'], ext_CWA_h[bucket]['sem3_h'], ext_CWA_h[bucket]['sem10_h'] = ext_CWA_h[bucket]['sem1_h'] + \
                                    s1, ext_CWA_h[bucket]['sem3_h'] + s3, ext_CWA_h[bucket]['sem10_h'] + s10
                                sem_h_triples_ext[bucket] += 1
                    ...
                    if self.sem == 'extensional' or self.sem == 'both':
                        if r.item() in self.dataset.r2hs.keys():
                            if len(self.dataset.r2hs[r.item()]) >= 10:
                                ...
                                ext_CWA_h[bucket_h]['sem1_h'], ext_CWA_h[bucket_h]['sem3_h'], ext_CWA_h[bucket_h]['sem10_h'] = ext_CWA_h[
                                    bucket_h]['sem1_h'] + s1, ext_CWA_h[bucket_h]['sem3_h'] + s3, ext_CWA_h[bucket_h]['sem10_h'] + s10
                                sem_h_triples_ext[bucket_h] += 1
                        if r.item() in self.dataset.r2ts.keys():
                            if len(self.dataset.r2ts[r.item()]) >= 10:
                                s1, s3, s5, s10 = self.sem_at_k_ext(
                                    indices_tail[:10], r.item(), side='tail', k=10)
                                ext_CWA_t[bucket_t]['sem1_t'], ext_CWA_t[bucket_t]['sem3_t'], ext_CWA_t[bucket_t]['sem10_t'] = ext_CWA_t[
                                    bucket_t]['sem1_t'] + s1, ext_CWA_t[bucket_t]['sem3_t'] + s3, ext_CWA_t[bucket_t]['sem10_t'] + s10
                                sem_t_triples_ext[bucket_t] += 1
        ...
        if (self.sem == 'extensional' or self.sem ==
            ...
            ext_CWA = {}
            for b in range(1, self.args.buckets + 1):
                ext_CWA_h[b]['sem1_h'] /= sem_h_triples_ext[b]
                ext_CWA_t[b]['sem1_t'] /= sem_t_triples_ext[b]
                ext_CWA_h[b]['sem3_h'] /= sem_h_triples_ext[b]
                ext_CWA_t[b]['sem3_t'] /= sem_t_triples_ext[b]
                ext_CWA_h[b]['sem10_h'] /= sem_h_triples_ext[b]
                ext_CWA_t[b]['sem10_t'] /= sem_t_triples_ext[b]
                ext_CWA[b] = {}
                ext_CWA[b]['sem1'] = (
                    ext_CWA_h[b]['sem1_h'] + ext_CWA_t[b]['sem1_t']) / 2
                ext_CWA[b]['sem3'] = (
                    ext_CWA_h[b]['sem3_h'] + ext_CWA_t[b]['sem3_t']) / 2
                ext_CWA[b]['sem10'] = (
                    ext_CWA_h[b]['sem10_h'] + ext_CWA_t[b]['sem10_t']) / 2

        if self.metric == 'sem' or self.metric == 'all':
            return filt_mrr_h, filt_mrr_t, filt_mrr, filt_hit_h, filt_hit_t, filtered_hits_at_1, filtered_hits_at_3, filtered_hits_at_10, \
                schema_CWA_h, schema_CWA_t, schema_CWA, ext_CWA_h, ext_CWA_t, ext_CWA
